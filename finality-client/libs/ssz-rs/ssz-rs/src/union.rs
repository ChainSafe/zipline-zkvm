use crate::{
    ...
    lib::*,
    merkleization::{mix_in_selector, mix_in_selector_tree, MerkleizationError, Merkleized, Node},
    ...
    fn to_merkle_tree(&mut self) -> Result<Vec<([u8; 32], [u8; 64])>, MerkleizationError> {
        match self {
            Some(value) => {
                ...
                let hash_tree_root = value.hash_tree_root()?;
                let pair = mix_in_selector_tree(&hash_tree_root, 1);
                tree.push(pair);
                Ok(tree)
            }
            None => Ok(vec![mix_in_selector_tree(&Node::default(), 0)]),
        }
    }
}
